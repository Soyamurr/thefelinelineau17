const ASSETS = {
  backgroundGif: "assets/bg.PNG",
  enemyGif: "assets/enemy.GIF",
  cats: {
    pink: { idle: "assets/murmur.GIF", shoot: null },
    blue: { idle: "assets/furshooter.GIF", shoot: "assets/furshooter1.GIF" },
    teal: { idle: "assets/wallcat.GIF", shoot: null },
    mix_purple: { idle: "assets/murshooter.GIF", shoot: "assets/murshooter1.GIF" },
    mix_rose: { idle: "assets/murwall.GIF", shoot: null },
    mix_azure: { idle: "assets/furwall.GIF", shoot: "assets/furwall1.GIF" }
  },
...
const BREEDS = {
  pink: { id:"pink", name:"Mur-Mur", role:"economy", color:"#E7AB4D", cost:5, incomeAmount:5, incomeInterval:5, radius:20, fn:"+5 pts every 5s." },
  blue: { id:"blue", name:"Fur-Shooter", role:"shooter", color:"#5D5E5D", cost:10, dmg:10, atkTime:1, bulletSpeed:260, rowTolerance:26, radius:22, fn:"Shoots enemies in its row." },
  teal: { id:"teal", name:"Wall-Cat", role:"wall", color:"#F9DBAE", cost:10, livesMax:3, hpPerLife:32, radius:22, fn:"Blocks enemies; 3 lives." }
};

const key = (a, b) => [a, b].sort().join("-");
const MERGE_WHEEL = {
  [key("blue", "pink")]: { id:"mix_purple", name:"Mur-Shooter", role:"shooter", color:"#A48653", dmg:15, atkTime:.7, bulletSpeed:280, rowTolerance:26, radius:25, incomeBonus:{amount:3,interval:5}, fn:"Shooter + income." },
  [key("pink", "teal")]: { id:"mix_rose", name:"Mur-Wall", role:"wall", color:"#F0C37E", livesMax:5, hpPerLife:32, radius:25, incomeBonus:{amount:3,interval:5}, fn:"5 lives + income." },
  [key("blue", "teal")]: { id:"mix_azure", name:"Fur-Wall", role:"shooter", color:"#A29581", dmg:15, atkTime:.7, bulletSpeed:280, rowTolerance:26, radius:25, livesMax:5, hpPerLife:32, fn:"Strong shooter + 5 lives." }
};
...
function draw() {
  ...
  ctx.fillStyle = "#498b75"; ctx.fillRect(6, 6, 18, H - 12);
...
function drawEnemy(e) {
  const r = e.radius;
  if (!imageDraw(ASSETS.enemyGif, e.x - r, e.y - r, r * 2, r * 2)) { ctx.fillStyle = "#792846"; ctx.fillRect(e.x - r, e.y - r, r * 2, r * 2) }
  ctx.fillStyle = "#eee"; ctx.fillRect(e.x - r, e.y - r - 8, r * 2, 4);
  ctx.fillStyle = "#792846"; ctx.fillRect(e.x - r, e.y - r - 8, r * 2 * Math.max(0, e.hp / e.maxHp), 4);
}
